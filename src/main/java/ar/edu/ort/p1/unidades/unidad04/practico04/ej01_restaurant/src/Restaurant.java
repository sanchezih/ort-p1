...
public class Restaurant {
	...
	public InfoPedido[] informeDePedidos() {
		InfoPedido[] infos = new InfoPedido[pedidos.size()];
		String apellido;
		int nivel;

		for (int i = 0; i < infos.length; i++) {
			Pedido pedido = this.pedidos.get(i);
			apellido = this.cajeros[pedido.getIdCajero()].getApellido();
			...
			infos[i] = new InfoPedido(pedido.getMonto(), apellido, nivel);
		}
		return infos;
	}
	 ...
	 * @return
	 */
	private Robot buscarRobot(String codigo) {
		Robot robot = null;
		int posRobot = idxRobot(codigo);
		if (posRobot > 0) {
			robot = this.robots[posRobot];
		}
		return robot;
	}
	 ...
	 * @return
	 */
	private int idxRobot(String codigo) {
		int i = this.robots.length - 1;
		while (i >= 0 && this.robots[i].getCodigo() != codigo) {
