...
public class Cabina {
	...
	private static final double INCREMENTO = 8;
	private int id;
	...
	public double cobrar(Vehiculo v) {
		...
		if (isHoraPico()) {
			res = v.getCategoria().tarifaBase + (v.getCategoria().tarifaBase * (Cabina.INCREMENTO / 100));
		}
		...
		if (this.mp instanceof MedioDePagoElectronico) {
			...
			res = res - (v.getCategoria().tarifaBase * (this.mp.obtenerDescuento() / 100));
		} else {
			System.out.println("El medio de pago de la cabina es " + this.mp.getClass().getSimpleName()
